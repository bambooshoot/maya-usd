    ...
    MPxFileTranslator::FileAccessMode /*mode*/)
{
    ...
    // zhusun modified begin
    for (auto dagPath : addedDagPaths) { 
        MGlobal::displayInfo(dagPath.fullPathName());

        MDagPathArray pathArray;
        dagPath.getAllPathsBelow(pathArray);
        for (auto subDagPath : pathArray) { 
            MObject shapeObj = subDagPath.node();
            MObject transObj = subDagPath.transform();

            MFnDagNode shapeNodeFn(shapeObj);
            MFnDagNode transNodeFn(transObj);
            MFnDagNode parentTransNodeFn(transNodeFn.parent(0));

            MString shapeName = transNodeFn.name();

            MDagModifier dagModifier;

            dagModifier.reparentNode(shapeObj, parentTransNodeFn.object());
            dagModifier.deleteNode(transObj);
            dagModifier.doIt();

            shapeNodeFn.setName(shapeName);
        }
    }
    // zhusun modified end
